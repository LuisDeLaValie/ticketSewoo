...
//		posPtr = new ESCPOSPrinter("BIG5"); // Big5
    ...
    void propio(){
        ...
        try {
            ...
            for (int i = 0; i < lista.length ; i++) {
                String s1=lista[i][0],s2=lista[i][1],s3=lista[i][2],s4=lista[i][3];
                doble(s1+"  ", s2 + " \t " + s3 + " \t " + s4);
                posPtr.lineFeed(1);
            }
            posPtr.lineFeed(1);
    ...
    void barcode(String s) throws UnsupportedEncodingException{
        posPtr.printBarCode(s, LKPrint.LK_BCS_Codabar, 40, 2, LKPrint.LK_ALIGNMENT_CENTER, LKPrint.LK_HRI_TEXT_BELOW);
    }
    void QRCode(String s)throws UnsupportedEncodingException{
        posPtr.printQRCode(s, s.length(), 5, ESCPOSConst.LK_QRCODE_EC_LEVEL_L, ESCPOSConst.LK_ALIGNMENT_CENTER);
    }
